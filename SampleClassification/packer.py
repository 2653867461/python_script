...
def unpack_upx(file_path,upx_path):
...
def AdjustPacker(file_path,diec_path,try_unpack=True):
    ...
    for item in packer_info["values"]:
        type_name = item["name"]
        if item["type"] == "Packer":
            result["packer_type"] = type_name
            if unpack_class.get(type_name,None) != None and unpack_class[type_name](file_path):
